...
export function WorkspaceShell({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-[100dvh] bg-background">
      <aside className={`fixed inset-y-0 left-0 z-40 flex w-[248px] flex-col bg-sidebar text-sidebar-foreground transition-transform duration-300 lg:translate-x-0 ${mobileOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="flex h-[82px] items-center justify-between border-b border-sidebar-border px-6">
          <Link href="/" className="flex items-center gap-3" data-testid="link-brand">
            <span className="grid size-9 place-items-center rounded-[10px] bg-sidebar-primary text-sidebar-primary-foreground shadow-lg">
            ...
            {navItems.map((item) => {
              ...
              return (
                <Link
                  ...
                  className={`group flex items-center gap-3 rounded-lg px-3 py-2.5 text-[13px] font-semibold transition-colors ${active ? 'bg-sidebar-accent text-white' : 'text-sidebar-foreground/65 hover:bg-sidebar-accent/70 hover:text-white'}`}
                >
      ...
      <main className="min-h-[100dvh] lg:pl-[248px]">
        <header className="sticky top-0 z-20 flex h-[70px] items-center justify-between border-b border-border/80 bg-background/90 px-5 backdrop-blur-md sm:px-8">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="font-mono-ui text-[10px] uppercase tracking-[0.18em] text-muted-foreground">Kestrel / {current?.label ?? 'Workspace'}</p>
              <h1 className="mt-0.5 font-display text-[17px] font-bold tracking-[-0.02em] text-foreground">{current?.label ?? 'Workspace'}</h1>
            </div>
          </div>
  ...
  return (
    <div className="mb-8 flex flex-col gap-5 md:flex-row md:items-end md:justify-between">
      <div>
        ...
        <h2 className="font-display text-[30px] font-bold leading-[1.05] tracking-[-0.04em] text-foreground sm:text-[36px]">{title}</h2>
        <p className="mt-3 max-w-[620px] text-[13px] leading-6 text-muted-foreground">{description}</p>
      </div>
      {action}
    </div>
  );
