...
import { ArrowRight, Check, ChevronLeft, ChevronRight, Database, FileSpreadsheet, Search, UploadCloud, X } from 'lucide-react';
...
function parseCsv(text: string): ClientInput[] {
...
function ImportPanel({ onDone }: { onDone: () => void }) {
  ...
  const submit = () => {
    ...
    importClients.mutate({ data: { fileName, rows } }, { onSuccess: (result) => { setMessage(`${result.imported} rows imported${result.skipped ? `, ${result.skipped} skipped` : ''}.`); onDone(); }, onError: () => setMessage('Import could not be completed. Check the file columns and retry.') });
  };
...
export default function Clients() {
  ...
  const query = useListClients({ search: search || undefined, page, pageSize: PAGE_SIZE }, { query: { queryKey: getListClientsQueryKey({ search: search || undefined, page, pageSize: PAGE_SIZE }) } });
  ...
  return <div className="animate-fade"><PageIntro eyebrow="LMS client register" title="Know every record." description="Search the imported loan book before preparing a CKYC file. The register is the source of truth for what enters a request." action={<button onClick={() => setImportOpen((open) => !open)} className="inline-flex items-center justify-center gap-2 rounded-lg bg-primary px-4 py-2.5 text-[12px] font-bold text-primary-foreground shadow-sm transition-transform hover:-translate-y-0.5" data-testid="button-toggle-import"><UploadCloud size={15} /> Import LMS rows</button>} />
    {importOpen && <div className="mb-6 animate-rise"><ImportPanel onDone={() => { setImportOpen(false); query.refetch(); }} /></div>}
    <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div className="relative max-w-[440px] flex-1"><Search size={16} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-muted-foreground" /><input value={search} onChange={(event) => { setSearch(event.target.value); setPage(1); }} placeholder="Search by name, loan ID, PAN or Client ID" className="h-11 w-full rounded-lg border border-input bg-card pl-10 pr-4 text-[12px] outline-none transition-colors placeholder:text-muted-foreground/70 focus:border-primary focus:ring-2 focus:ring-primary/15" data-testid="input-search-clients" /></div><div className="flex items-center gap-2 font-mono-ui text-[10px] uppercase tracking-[.11em] text-muted-foreground"><Database size={14} className="text-primary" /> {showing}</div></div>
     {query.isError ? <QueryError onRetry={() => query.refetch()} /> : query.isLoading ? <div className="overflow-hidden rounded-xl border border-border bg-card"><div className="space-y-3 p-5">{[1,2,3,4,5,6].map((i) => <div className="h-12 animate-pulse rounded-lg bg-muted" key={i} />)}</div></div> : clients.length === 0 ? <EmptyState icon={Database} title={search ? 'No matching clients' : 'Your client register is empty'} detail={search ? 'Try a different name, loan ID, PAN, or Client ID.' : 'Import an LMS CSV export to create the working register.'} action={!search && <button onClick={() => setImportOpen(true)} className="inline-flex items-center gap-2 rounded-lg bg-primary px-4 py-2 text-[12px] font-bold text-primary-foreground" data-testid="button-empty-import"><UploadCloud size={14} /> Import rows</button>} /> : <div className="overflow-hidden rounded-xl border border-border bg-card shadow-xs"><div className="overflow-x-auto"><table className="data-table w-full min-w-[880px] text-left"><thead className="bg-secondary/55"><tr className="border-b border-border text-muted-foreground"><th className="px-5 py-3.5 font-medium">Client</th><th className="px-4 py-3.5 font-medium">Loan ID</th><th className="px-4 py-3.5 font-medium">PAN</th><th className="px-4 py-3.5 font-medium">Contact</th><th className="px-4 py-3.5 font-medium">Disbursed</th><th className="px-4 py-3.5 font-medium">Added</th><th className="px-3 py-3.5" /></tr></thead><tbody className="divide-y divide-border">{clients.map((client) => <tr className="group transition-colors hover:bg-secondary/30" key={client.id} data-testid={`row-client-${client.id}`}><td className="px-5 py-4"><div className="flex items-center gap-3"><span className="grid size-8 shrink-0 place-items-center rounded-full bg-[#dcefeb] font-mono-ui text-[10px] font-medium text-primary">{client.ClientName.split(' ').map((part) => part[0]).slice(0,2).join('')}</span><div><p className="text-[12px] font-semibold text-foreground">{client.ClientName}</p><p className="mt-0.5 font-mono-ui text-[10px] text-muted-foreground">{client.ClientID}</p></div></div></td><td className="px-4 font-mono-ui text-[11px] text-foreground/75">{client.loanid}</td><td className="px-4 font-mono-ui text-[11px] text-foreground/75">{client.Client_PAN || '—'}</td><td className="px-4"><p className="font-mono-ui text-[11px] text-foreground/75">{client.mobile_no || '—'}</p><p className="mt-0.5 text-[10px] text-muted-foreground">{client.Gender} · {client.date_of_birth}</p></td><td className="px-4 font-mono-ui text-[10px] text-muted-foreground">{client.disbursedon_date}</td><td className="px-4 font-mono-ui text-[10px] text-muted-foreground">{new Intl.DateTimeFormat('en-IN', { day:'2-digit', month:'short' }).format(new Date(client.createdAt))}</td><td className="px-3"><Link href={`/requests?client=${client.id}`} className="grid size-8 place-items-center rounded-md text-muted-foreground opacity-0 transition-all hover:bg-secondary hover:text-primary group-hover:opacity-100" title="Use in request" data-testid={`link-use-client-${client.id}`}><ArrowRight size={15} /></Link></td></tr>)}</tbody></table></div><div className="flex items-center justify-between border-t border-border px-5 py-3"><p className="text-[11px] text-muted-foreground">Showing {showing}</p><div className="flex items-center gap-1"><button disabled={page <= 1} onClick={() => setPage((current) => current - 1)} className="grid size-8 place-items-center rounded-md border border-border text-muted-foreground disabled:opacity-30 hover:bg-secondary" aria-label="Previous page" data-testid="button-previous-page"><ChevronLeft size={15} /></button><span className="px-2 font-mono-ui text-[10px] text-muted-foreground">{page} / {pageCount}</span><button disabled={page >= pageCount} onClick={() => setPage((current) => current + 1)} className="grid size-8 place-items-center rounded-md border border-border text-muted-foreground disabled:opacity-30 hover:bg-secondary" aria-label="Next page" data-testid="button-next-page"><ChevronRight size={15} /></button></div></div></div>}
  </div>;
}
