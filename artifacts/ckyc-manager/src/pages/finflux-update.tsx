    ...
    <section className="overflow-hidden rounded-xl border border-border bg-card shadow-xs" aria-label="Finflux update job">
      <div className="border-b border-border bg-[#f1f7f5] px-5 py-4 dark:bg-card">
        ...
        <div className="mt-4 grid grid-cols-2 gap-2 sm:grid-cols-4">
          ...
          <div className="rounded-lg border border-border/70 bg-card px-3 py-2"><p className="font-mono-ui text-[9px] uppercase tracking-[.12em] text-muted-foreground">Not attempted</p><p className="mt-1 font-mono-ui text-[16px] font-semibold text-muted-foreground">{job.notAttemptedCount}</p></div>
        </div>
      </div>
        ...
        <div className="overflow-x-auto">
          <table className="data-table w-full min-w-[860px] text-left">
            ...
            <tbody className="divide-y divide-border">
              {job.results.map((result) => <tr key={`${result.rowNumber}-${result.clientId}`} className="text-[11px]"><td className="px-5 py-3 font-mono-ui text-muted-foreground">{result.rowNumber}</td><td className="px-3 py-3 font-mono-ui font-semibold">{result.clientId}</td><td className="px-3 py-3 font-mono-ui text-foreground/75">{result.ckycNumber}</td><td className="px-3 py-3">{result.attempted ? <StatusPill status={result.status} /> : <span className="inline-flex rounded-full bg-secondary px-2.5 py-1 font-mono-ui text-[9px] font-semibold uppercase tracking-[.12em] text-muted-foreground">Not attempted</span>}</td><td className="px-3 py-3 font-mono-ui text-foreground/75">{result.resourceId ?? '—'}</td><td className="max-w-[260px] px-3 py-3 text-muted-foreground">{result.message}{result.statusCode ? ` · ${result.statusCode}` : ''}</td><td className="px-3 py-3 font-mono-ui text-muted-foreground">{result.durationMs === null ? '—' : `${result.durationMs} ms`}</td></tr>)}
            </tbody>
          </table>
        </div>
      ) : (
                ...
                <>
                  <div className="overflow-x-auto"><table className="data-table w-full min-w-[780px] text-left"><thead className="bg-secondary/45"><tr className="border-b border-border text-muted-foreground"><th className="w-12 px-5 py-3"><span className="sr-only">Select</span></th><th className="px-3 py-3">Client</th><th className="px-3 py-3">Loan ID</th><th className="px-3 py-3">FinFlux status</th><th className="px-3 py-3">Outgoing CKYC number</th></tr></thead><tbody className="divide-y divide-border">{clients.map((client) => {
                    ...
                    return <tr key={client.id} className={`transition-colors ${selected ? 'bg-[#f4faf8]' : 'hover:bg-secondary/30'}`} data-testid={`row-finflux-client-${client.id}`}>
                      ...
                      <td className="px-3 py-4">{client.finfluxStatus === 'updated' ? <><span className="inline-flex rounded-full bg-[#dcf3e9] px-2 py-1 font-mono-ui text-[8px] font-semibold uppercase tracking-[.08em] text-[#31734d]">Updated</span>{client.finfluxResourceId && <p className="mt-1 font-mono-ui text-[9px] text-muted-foreground">Resource ID {client.finfluxResourceId}</p>}</> : client.finfluxStatus === 'failed' ? <><span className="inline-flex rounded-full bg-[#fff0ed] px-2 py-1 font-mono-ui text-[8px] font-semibold uppercase tracking-[.08em] text-[#a23e36]">Failed</span>{client.finfluxError && <p className="mt-1 max-w-[260px] line-clamp-2 text-[9px] leading-4 text-destructive" title={client.finfluxError}>{client.finfluxError}{client.finfluxStatusCode ? ` · ${client.finfluxStatusCode}` : ''}</p>}</> : client.finfluxStatus === 'pending' ? <span className="inline-flex rounded-full bg-[#fff0c9] px-2 py-1 font-mono-ui text-[8px] font-semibold uppercase tracking-[.08em] text-[#9d761f]">Pending</span> : <span className="inline-flex rounded-full bg-secondary px-2 py-1 font-mono-ui text-[8px] font-semibold uppercase tracking-[.08em] text-muted-foreground">Final CKYC required</span>}</td>
