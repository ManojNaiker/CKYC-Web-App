...
**How to apply:** Keep institution, total generated row count, version, and hyphenated file date in the type 10 record; do not put filename serial there. Aadhaar E rows use the last four digits plus name, date of birth, and gender; normalize every E-row DOB to `DD-MM-YYYY` even when LMS data is ISO or slash-formatted. B rows use the complete alternate identifier with empty trailing fields. Count generated E/B rows rather than selected clients when enforcing the 10-lakh limit.